...
BridgeRenderComponent::BridgeRenderComponent(Struct2f pos, World::WorldHeight height, unsigned int width, unsigned int length, TileUV data1, TileUV data2, SGRender::Renderer* ren, bool horizontal)
{
	using World::TILE_SIZE; using namespace Geometry; using namespace SGRender;
	...
	for (int z = 0; z < length; z++)
	{
		for (int x = 0; x < width; x++)
		{
			...
			if (alternate) 
			{
				...
				AxialRotate<NTVertex>((NTVertex*)&tileSide, { xPos, yPos, 0.0f }, -90.0f, Shape::QUAD, Axis::Z);
				AxialRotate<NTVertex>((NTVertex*)&tileSide, { xPos, yPos, 0.0f }, 90.0f, Shape::QUAD, Axis::Y);
				Translate<NTVertex>((NTVertex*)&tileSide, TILE_SIZE, 0.0f, zPos, Shape::QUAD);
				if (!horizontal)
				{
					alternate = false;
				}
			...
			else if (!alternate)
			{
				...
				AxialRotate<NTVertex>((NTVertex*)&tileSide, { xPos, yPos, 0.0f }, 90.0f, Shape::QUAD, Axis::Z); 
				AxialRotate<NTVertex>((NTVertex*)&tileSide, { xPos-32.0f, yPos, 0.0f }, 90.0f, Shape::QUAD, Axis::Y);
				Translate<NTVertex>((NTVertex*)&tileSide, TILE_SIZE, -9.0f, zPos, Shape::QUAD);
				if (!horizontal)
				{
					alternate = true;
				}
			...
			//Position Sprite
			AxialRotate<NTVertex>((NTVertex*)&tile, { x + TILE_SIZE / 2, yPos, 0.0f }, -90.0f, Shape::QUAD, Axis::X);
			Translate<NTVertex>((NTVertex*)&tile, 0.0f, 0.0f, zPos, Shape::QUAD); 
			
			//If horizontal, rotate about centre
			if (horizontal)
			{
				AxialRotate<NTVertex>((NTVertex*)&tile, { xPos + TILE_SIZE/2 , yPos, zPos-TILE_SIZE/2 }, -90.0f, Shape::QUAD, Axis::Y);
				AxialRotate<NTVertex>((NTVertex*)&tileSide, { xPos + TILE_SIZE / 2 , yPos, zPos - TILE_SIZE / 2 }, -90.0f, Shape::QUAD, Axis::Y);
			}

			//Gen normals
			CalculateQuadNormals((NTVertex*)&tile);
			CalculateQuadNormals((NTVertex*)&tileSide);
